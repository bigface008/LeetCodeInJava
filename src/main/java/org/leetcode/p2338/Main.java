package org.leetcode.p2338;

import java.util.ArrayList;
import java.util.List;

// https://leetcode.cn/problems/count-the-number-of-ideal-arrays/?envType=daily-question&envId=2025-04-22
class Solution {
    private static final int MOD = 1_000_000_007;
    private static final int MAX_N = 10_000;
    private static final int MAX_E = 13;

    private static final List<Integer>[] EXP = new ArrayList[MAX_N + 1];
    private static final int[][] C = new int[MAX_N + MAX_E][MAX_E + 1];

    private static boolean done = false;

    static {
        for (int x = 1; x < EXP.length; x++) {
            EXP[x] = new ArrayList<Integer>();
            int t = x;
            for (int i = 2; i * i <= t; i++) {
                int e = 0;
                for (; t % i == 0; t /= i) {
                    e++;
                }
                if (e > 0) {
                    EXP[x].add(e);
                }
            }
            if (t > 1) {
                EXP[x].add(1);
            }
        }

        for (int i = 0; i < MAX_N + MAX_E; i++) {
            C[i][0] = 1;
            for (int j = 1; j <= Math.min(i, MAX_E); j++) {
                C[i][j] = (C[i - 1][j] + C[i - 1][j - 1]) % MOD;
            }
        }
    }

    public int idealArrays(int n, int maxValue) {
        long ans = 0;
        for (int x = 1; x <= maxValue; x++) {
            long mul = 1;
            for (int e : EXP[x]) {
                mul = mul * C[n + e - 1][e] % MOD;
            }
            ans += mul;
        }
        return (int) (ans % MOD);
    }
}


//class Solution {
//    final long MOD = 1_000_000_007;
//    int n;
//    int maxValue;
//    List<Integer> group;
//    long ans = 0;
//
//    public int idealArrays(int n, int maxValue) {
//        this.n = n;
//        this.maxValue = maxValue;
//        group = new ArrayList<>(n);
//        dfs(0);
//        return (int) (ans % MOD);
//    }
//
//    private void dfs(int i) {
//        if (i == n) {
//            ans++;
//            return;
//        }
//        if (i == 0) {
//            for (int v = 1; v <= maxValue; v++) {
//                group.add(v);
//                dfs(i + 1);
//                group.remove(group.size() - 1);
//            }
//        } else {
//            int prev = group.get(group.size() - 1);
//            for (int v = prev; v <= maxValue; v += prev) {
//                group.add(v);
//                dfs(i + 1);
//                group.remove(group.size() - 1);
//            }
//        }
//    }
//}

public class Main {
}
