...
//        test(new int[]{1, 1, 0, 0, 0, 1, 1, 0, 0, 1}, 3, 1, 3);
//        test(new int[]{0, 0, 0, 0}, 2, 3, 4);
        test(new int[]{1, 0, 1}, 5, 3, 8);
    }

    private static void test(int[] nums, int k, int maxChanges, long expect) {
        Solution solution = new Solution();
        long output = solution.minimumMoves(nums, k, maxChanges);
        String desc = String.format("nums=%s, k=%d, maxChanges=%d", Arrays.toString(nums), k, maxChanges);
